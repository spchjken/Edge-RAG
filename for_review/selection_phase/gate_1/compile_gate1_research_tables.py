    ...
    def select_winning_configuration(self) -> Dict[str, Any]:
        ...
        for cfg in candidates:
            ...
            for ds in active_dev:
                ...
                clean_rechits = [r for r in rechits if not np.isnan(r)]
                ds_metrics.append({
                    "dataset": ds,
                    ...
                    "rechit": float(np.mean(clean_rechits)) if clean_rechits else np.nan,
                })
            ...
            valid_rechits = [m["rechit"] for m in ds_metrics if not np.isnan(m["rechit"])]
            macro_rechit = float(np.mean(valid_rechits)) if valid_rechits else 1.0
