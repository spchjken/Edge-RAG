...
> **Scope notice:** This is a historical mixed-scope benchmark catalog. Only its corpus, query,
> qrels, and first-stage retrieval preparation guidance applies to active CRVE work. Downstream
> cascade, LLM reranking, and generation discussions are archival context, not active CRVE goals.
> [ARCHITECTURE.md](ARCHITECTURE.md) owns current scope; frozen pool/Gate 1 numeric settings are in
> [`gate1_phase2_1a.yaml`](../configs/gate1_phase2_1a.yaml) and its `for_review/` copy.

This document catalogs Retrieval-Augmented Generation (RAG) and Information Retrieval (IR) benchmarks historically evaluated or considered in Edge-RAG.
...
1. **Historical full Edge-RAG suitability:** End-to-end generation and LLM components below are archival, outside active CRVE scope.
2. **Active CRVE suitability:** A benchmark needs a searchable corpus, queries, and relevance judgments for first-stage lexical retrieval; see [ARCHITECTURE.md](ARCHITECTURE.md).
...
2. **Unified Direct Ingestion (`src/evaluation/benchmark_loader.py`):**
   CRVE provides [`BenchmarkLoader`](../src/evaluation/benchmark_loader.py), which streams directly from raw `corpus.jsonl` and `qrels/test.tsv` without flat-file filesystem fragmentation or filename collisions:
   ```python
...
The primary benchmark matrix for formal Information Retrieval baselining and reproduction, executed via [`CRVE/scripts/run_pyterrier_baselines.py`](../scripts/run_pyterrier_baselines.py) with zero raw-corpus RAM retention (`BenchmarkLoader.stream_corpus`):
...
Historically generated by `scripts/benchmark_creation/` (not present in the active CRVE checkout) across domains `ai`, `biomedical`, `fintech`, `fused`, and `systems_security`, under tiers `corpus_single_*`, `corpus_multi_*`, and `corpus_stress_*`.
