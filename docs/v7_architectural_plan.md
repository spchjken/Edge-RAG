...
* **Upgrade 1.2 — Coverage-Based Vocabulary Pool (Static, Index-Time):**
  - Build the pool from the **full analyzed stem vocab (no DF floor)** and select $N_{\text{vocab}}$ by **farthest-point sampling (FPS)** over the $L_2$-normalized stem embeddings — coverage/hub selection replacing the retired frequency salience ($\text{IDF} \times \ln(1+\text{DF})$ over $\text{DF}\ge 2$). Rationale and corrected analysis in [`pool_design_options.md`](pool_design_options.md).
